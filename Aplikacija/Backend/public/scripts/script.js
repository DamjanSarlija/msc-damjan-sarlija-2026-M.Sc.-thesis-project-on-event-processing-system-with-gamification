...
function prikaziPodatak(item, baza = false) {
    if (prikazaniIdevi.has(item.podatak_id)) {
        return;
    } else {
        prikazaniIdevi.add(item.podatak_id);
        const list = document.getElementById('data-list');
        const placeholder = list.querySelector('.no-data');
        if (placeholder) placeholder.remove();

        const li = document.createElement('li');
        li.dataset.podatakId = item.podatak_id;
        if (baza) {
            li.textContent = `✅ Uređaj ${item.uredjaj_id}: ${item.podatak} — ${new Date(item.vrijeme).toLocaleTimeString()}`;
        } else {
            li.textContent = `⌛ Uređaj ${item.uredjaj_id}: ${item.podatak} — ${new Date(item.vrijeme).toLocaleTimeString()}`;            
        }
...
socket.on("database_data", (item) => {
    const li = document.querySelector(`[data-podatak-id="${item.podatak_id}"]`);
    if (li) {
